...
void checknode(){
//  if(dLs && dRs && ir[4] && ir[5] && !ir[2] &&  !ir[3]){
//    Serial.println("Hogya1");
//    hogya();
//  }
//  else if(!dLs && !dRs && !ir[4] && !ir[5] && !ir[2] && ! ir[3]){
//    Serial.println("Hogya2");
//    hogya();
//  }
//  else if(dLs && dRs && !ir[4] && !ir[5] && !ir[2] && ! ir[3]){
//    Serial.println("Hogya3");
//    hogya();
//  }
//  else if(dLs && !dRs && (((ir[1] && ir[2] && ir[3] && ir[4]) || (ir[2] && ir[3] && ir[4] && ir[5]) || (ir[3] && ir[4] && ir[5] && ir[6]) || (ir[1] && ir[2] && ir[3]) || (ir[2] && ir[3] && ir[4]) || (ir[3] && ir[4] && ir[5]) || (ir[4] && ir[5] && ir[6])))){
//    Serial.println("Hogya3");
//    hogya();
//  }
//  else if(dLs && !dRs && ir[4] && ir[5] && !ir[2] &&  !ir[3]){
//    Serial.println("Hogya4");
//    hogya();
//  }

  if(ir[2]==ir[3] && ir[4]==ir[5] && (dLs || dRs) && dFr)
  {
     ...
     node_arr[1][0] /= cnt;
     node_arr[1][1] /= cnt;
     if(node_arr[0][0]==in_3 && node_arr[0][1]==in_4 && node_arr[1][0]==in_2 && node_arr[1][1]==in_1)
     {
      Serial.println("Node confirmed");
      char buffer[40];
    sprintf(buffer, "%d --- %d", node_arr[0][0], node_arr[0][1]);
    Serial.println(buffer);
    sprintf(buffer, "%d --- %d", node_arr[1][0], node_arr[1][1]);
    Serial.println(buffer);
    done();
     }
  }
//  if(ir[2]==ir[3] && ir[4]==ir[5] && (!dLs && !dRs) && dFr){
//     Serial.println("all white node detected");
//  }
}
...
//  while (!digitalRead(ir1) !digitalRead(ir3) && !digitalRead(ir4) && !digitalRead(ir5) && !digitalRead(ir6) && !digitalRead(ir7)) {
//    analogWrite(ML1, v);
//    analogWrite(ML2, 0);
//    analogWrite(MR1, 0);
//    analogWrite(MR2, v);
